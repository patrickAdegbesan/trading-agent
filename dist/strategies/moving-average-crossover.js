...
class MovingAverageCrossoverStrategy extends base_strategy_1.BaseStrategy {
    constructor(config = {
        fastPeriod: 12,
        slowPeriod: 26,
        ...
        if (signal !== 'NONE') {
            ...
            if (this.signalCount >= this.config.confirmationPeriod &&
                ...
                signals.push({
                    ...
                    stopLoss: this.calculateStopLoss(price, atr, signal),
                    takeProfit: this.calculateTakeProfit(price, atr, signal, confidence),
    ...
    calculateConfidence(crossoverStrength, rsi, side) {
    ...
    calculateStopLoss(price, atr, side) {
        const atrMultiplier = 2.0; // 2x ATR for stop loss
        return side === 'BUY'
            ? price - (atr * atrMultiplier)
            : price + (atr * atrMultiplier);
    }
    calculateTakeProfit(price, atr, side, confidence) {
    ...
    evaluatePerformance(trades) {
        ...
        const returnsStd = Math.sqrt(returns.reduce((sum, ret) => sum + Math.pow(ret - returnsMean, 2), 0) / returns.length);
...
//# sourceMappingURL=moving-average-crossover.js.map