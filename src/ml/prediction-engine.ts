...
export class PredictionEngine {
    ...
    private confidenceAdjustment: Map<string, number> = new Map(); // Symbol-specific confidence adjustments
    private recentPredictionAccuracy: Map<string, number[]> = new Map(); // Track recent prediction accuracy
    ...
    private initializeConfidenceTracking(): void {
     ...
     */
    public generateSignal(input: PredictionInput): TradingSignal | null {
        try {
     ...
     */
    private calculateTechnicalSignal(input: PredictionInput): { side: 'BUY' | 'SELL', confidence: number } {
     ...
     */
    private calculateMLConfidence(symbol: string, baseConfidence: number): number {
     ...
     */
    private applyPerformanceAdjustment(symbol: string, confidence: number): number {
     ...
     */
    private estimateExpectedReturn(input: PredictionInput): number {
        if (!this.databaseService) {
     ...
     */
    private estimateWinProbability(symbol: string, side: 'BUY' | 'SELL'): number {
        if (!this.databaseService) {
        ...
        try {
            ...
            const sameSideTrades = recentTrades.filter(trade => 
                trade.side === side && trade.pnl !== undefined
            );
     ...
     */
    private calculateDynamicStopLoss(input: PredictionInput): number {
     ...
     */
    private calculateDynamicTakeProfit(input: PredictionInput): number {
     ...
     */
    public updatePerformanceMetrics(symbol: string, predictedSuccess: boolean, actualSuccess: boolean): void {
     ...
     */
    private getRecentAccuracy(symbol: string): number {
        const recentAccuracy = this.recentPredictionAccuracy.get(symbol) || [];
     ...
     */
    private logPrediction(signal: TradingSignal): void {
     ...
     */
    public getPerformanceStats(): { [symbol: string]: any } {
